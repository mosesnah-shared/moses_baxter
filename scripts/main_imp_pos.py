...
from my_controllers  import JointPositionController, JointImpedanceController, CartesianImpedanceControllerPos
...
def main():
    ...
    impR_2 = CartesianImpedanceControllerPos( my_baxter, which_arm = "right", name = "right_imp2", is_save_data = False )
    impR_2.set_linear_impedance( Kx = Kx_mat, Bx = Bx_mat )
                                    
    # Add the movements                               
    impR_2.add_linear_movement( xp0i = C.GRASP_POSE_UP_EE_POS_INIT, xp0f = C.GRASP_POSE_UP_EE_POS_INIT + np.array( [ 0.0, -0.3, 0.0 ] ) , D = 3.0, ti = 0.0 )
    impR_2.add_linear_movement( xp0i = np.zeros( 3 )              , xp0f = np.array( [ 0.0,  0.0,  0.3 ] )    , D = 3.0, ti =  3.5 )
    impR_2.add_linear_movement( xp0i = np.zeros( 3 )              , xp0f = np.array( [ 0.0,  0.3,  0.0 ] )    , D = 3.0, ti =  7.0 )
    impR_2.add_linear_movement( xp0i = np.zeros( 3 )              , xp0f = np.array( [ 0.0,  0.0, -0.3 ] )    , D = 3.0, ti = 10.5 )
    impR_2.add_linear_movement( xp0i = np.zeros( 3 )              , xp0f = np.array( [ 0.0, -0.3,  0.0 ] )    , D = 3.0, ti = 14.0 )
    impR_2.add_linear_movement( xp0i = np.zeros( 3 )              , xp0f = np.array( [ 0.0,  0.0,  0.3 ] )    , D = 3.0, ti = 17.5 )
    impR_2.add_linear_movement( xp0i = np.zeros( 3 )              , xp0f = np.array( [ 0.0,  0.3,  0.0 ] )    , D = 3.0, ti = 21.0 )        
    impR_2.add_linear_movement( xp0i = np.zeros( 3 )              , xp0f = np.array( [ 0.0,  0.0, -0.3 ] )    , D = 3.0, ti = 24.5 )                
            
    # The 2nd Impedance
    tmp_left_pos = np.zeros( 3 )
    tmp_left_pos[ 0 ] =  C.GRASP_POSE_UP_EE_POS_INIT[ 0 ]
    tmp_left_pos[ 1 ] = -C.GRASP_POSE_UP_EE_POS_INIT[ 1 ]
    tmp_left_pos[ 2 ] =  C.GRASP_POSE_UP_EE_POS_INIT[ 2 ]
    
    impL_2 = CartesianImpedanceControllerPos( my_baxter, which_arm = "left", name = "left_imp2" )
    impL_2.set_linear_impedance( Kx = Kx_mat, Bx = Bx_mat )
    
    impL_2.add_linear_movement( xp0i = tmp_left_pos     , xp0f = tmp_left_pos + np.array( [ 0.0, 0.3, 0.0 ] )          , D = 3.0, ti = 0.0  )
    impL_2.add_linear_movement( xp0i = np.zeros( 3 )    , xp0f = np.array( [ 0.0, -0.15,  0.15 * np.sin( np.pi/3 ) ] ) , D = 3.0, ti = 3.5  )
    impL_2.add_linear_movement( xp0i = np.zeros( 3 )    , xp0f = np.array( [ 0.0, -0.15, -0.15 * np.sin( np.pi/3 ) ] ) , D = 3.0, ti = 7.0  )
    impL_2.add_linear_movement( xp0i = np.zeros( 3 )    , xp0f = np.array( [ 0.0, 0.3, 0.0 ] )                         , D = 3.0, ti = 10.5 )
    impL_2.add_linear_movement( xp0i = np.zeros( 3 )    , xp0f = np.array( [ 0.0, -0.15,  0.15 * np.sin( np.pi/3 ) ] ) , D = 3.0, ti = 14.0 )
    impL_2.add_linear_movement( xp0i = np.zeros( 3 )    , xp0f = np.array( [ 0.0, -0.15, -0.15 * np.sin( np.pi/3 ) ] ) , D = 3.0, ti = 17.5 )   
