...
% All the name/value are inside the square bracket, and it is formatted as:
...
while( ~feof( fid ) )
    ...
    for i = 1: round( length( tmp )/2 )
        tmpField = tmp{ 2 * i - 1}( 2 : end - 1 );                                    % Getting the name of the data
        tmpField = tmpField( ~isspace( tmpField ) );
        tmpValue = str2double( tmp{ 2 *i  }( 2 : end - 1 ) );
